# Maximized windows: revised copy

Approved for publication by Jon on September 22, 2026, including the penny-allocation and myFPL report explanations. These passages appear only when their window is maximized. The approved compact copy remains unchanged.

The rendered copy lives in `content/window-details.ts`; keep this review copy in sync when editing.
...
I use AI tools heavily in building these apps. A lot of my work is deciding what should happen, working through the weird cases, and checking that what comes back actually does the job correctly, will continue to do its job correctly at reasonable scale, and won’t cost me a billion dollars, *ever*. Getting something onto a screen is a lovely start, but there's usually quite a bit left to figure out.
...
Splitting bills has lots of interesting problems to solve, like what to do with all the pennies! Split $10 three ways and somebody needs to owe $3.34. Who should it be? The app uses the details of the expense to pick a starting point in the group, then hands out the leftover pennies from there. The same bill gets the same split, but the starting point can change from bill to bill, so being first on the list doesn't automatically make you the extra-penny person. For percentage splits, it uses the [largest remainder method](https://www.census.gov/about/history/historical-censuses-and-surveys/census-programs-surveys/decennial-census/methods.html): round each share down to whole cents, then give the leftover pennies to the shares that lost the biggest fractions. Either way, the total still adds up.
...
### Economics and quite a lot of code
...
### The explanation is part of learning it
...
## Editing notes retained from review

- Applied your revisions, keeping intentional phrasing and grammar. Experience remains unchanged. The footer keeps “You maximized the footer”, without “Huzzah!”.

- The hero’s “ever” is italicized. Removed bracketed instructions, the literal `&#x20;` paste artifacts, and the doubled space after “here.” in PayTogether. No other typo or grammar corrections.

- Drafted the penny paragraph after “Who should it be?” The explanation distinguishes the expense-based starting point for even splits from largest-remainder allocation for percentage splits. The linked Census Bureau page describes the latter mathematical method, not the app’s choice of starting point. The code does not promise a strict rotation across successive bills.

- Replaced the first two proposed myFPL report sentences with the three sentences beginning “Before the AI gets involved”. The rankings and evaluated options are calculated in code, but the report model is allowed to weigh trade-offs and justify departures. The passage therefore avoids claiming that every written recommendation is deterministic.
